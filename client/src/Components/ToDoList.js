...
function ToDoList() {
  ...
  const handleTaskUpdate = () => {
    ...
    if (editedTask.value.trim() !== '') {
      ...
      Axios.put(`https://nodejsdep-piratekingprems-projects.vercel.app/api/v1/task/${editedTask.id}`, { task: editedTask.value })
        .then((response) => {
  ...
  const handleTaskDelete = (e) =>{
      Axios.delete(`https://nodejsdep-piratekingprems-projects.vercel.app/api/v1/task/${e}`)
      .then(
        fetchTasks()
      ).catch((error)=>{
        console.log(error);
  ...
  const handleSubmit = (e) => {
    ...
    if (inputData.task.trim() !== '') {
      Axios.post('https://nodejsdep-piratekingprems-projects.vercel.app/api/v1/task', inputData)
        .then((response) => {
          console.log(response);
  ...
  const fetchTasks = async () => {
    try {
      const { data } = await Axios.get(`https://nodejsdep-piratekingprems-projects.vercel.app/api/v1/task/${user}`);
  ...
  const handleChangeOfCheckBox = (e) => {
    e.preventDefault();
    if (e.target.checked) {
      Axios.put(`https://nodejsdep-piratekingprems-projects.vercel.app/api/v1/task/status/${e.target.value}`, status)
        .then((response) => {
          console.log(response);
          // Fetch the updated list of tasks
          fetchTasks();
